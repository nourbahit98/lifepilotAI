...
export function AppShell({
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
        ...
        <aside className="hidden lg:block">
          <nav className="sticky top-24 space-y-1">
            {navItems.map(([label, href]) => (
              <Link
                className="block rounded-lg px-4 py-3 text-sm font-medium text-slate-600 hover:bg-white hover:text-slate-950"
                href={href}
                key={href}
              >
                {label}
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 grid grid-cols-5 border-t border-slate-200 bg-white/95 px-3 py-2 text-center text-xs font-medium text-slate-600 backdrop-blur lg:hidden">
        {navItems.slice(0, 5).map(([label, href]) => (
          <Link className="rounded-lg px-1 py-2 hover:bg-slate-100" href={href} key={href}>
            {label}
          </Link>
        ))}
      </nav>
